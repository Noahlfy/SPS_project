...
mqtt_client_4 = MQTTClient("localhost", "esp32/output4", data_handler)
mqtt_client_5 = MQTTClient("localhost", "esp32/output5", data_handler)
...
def start_mqtt_clients(session_id):
    ...
    threading.Thread(target=mqtt_client_4.start, daemon=True).start()   
    threading.Thread(target=mqtt_client_5.start, daemon=True).start()
